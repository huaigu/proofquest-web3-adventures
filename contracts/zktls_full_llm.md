...
At the end of the onboarding flow, you can enter a referral code received from other users to earn extra reward points. If you donâ€™t have a referral code at first, you can still enter it later on the Achievements page through the task â€œSign-in using a referral codeâ€ and claim your points.
...
All earned points are displayed in detail in the Achievements pageâ€™s rewards history.
...
With zkTLS, crypto payment can be further simplified and abstracted away from wallet addresses. Instead of requiring a recipientâ€™s wallet address, senders can specify a social account (e.g., email, Twitter, GitHub) as the payment destination. The recipient can later prove ownership of that account via zkTLS and securely claim the tokens into their own wallet.
...
With zkTLS-based crypto payments, users no longer need to manage or disclose their wallet addresses. Instead, they can receive assets by simply proving ownership of an existing identityâ€”such as a social account or emailâ€”without exposing sensitive information on-chain. This approach ensures end-to-end privacy, as the userâ€™s identity remains completely hidden from the public ledger, reducing both privacy risks and friction in participation.
...
- ï¼ˆ1ï¼‰For attestationTypeID 9 & 11
...
- ï¼ˆ2ï¼‰For attestationTypeID 10 & 12
...
- ï¼ˆ3ï¼‰For attestationTypeID 15
...
- ï¼ˆ4ï¼‰For attestationTypeID 16 & 17
...
- ï¼ˆ5ï¼‰For attestationTypeID 101
...
The attestationParameters should include a signature from the userâ€™s wallet address, which needs to be attested to confirm ownership of the address, along with a timestamp indicating the time of the signature. **The input should follow this order: first, â€˜user signatureâ€™; second, â€˜timestampâ€™**.
...
:::note  
In order to confirm that the user truly owns the address to be attested, you must verify that the transmitted â€˜user signatureâ€™ was signed from that address. The verification method is:
...
- **Hashed result**
  ...
  By default, our zkTLS SDK retrieves a plaintext verification result. If, in some cases, you want to keep the userâ€™s data private, a hashed verification result is preferred. In the hashed result case, a â€˜SHA256â€™ hashed data item will be returned to your DApp.
...
However, if your application also runs on mobile devices, you must include this parameter. It is used to detect the userâ€™s device type, such as **Android, iPhone, or PC**. If you want to restrict usage to Android devices only, set the value to 'android'.

Currently, only Android devices are supported. iOS is coming soon.

```javascript
// Set the device parameter to detect the userâ€™s device type when your application supports both PC and mobile. Currently, only Android devices are supported. iOS is coming soon.
...
Hereâ€™s a basic example of how to configure and initialize the Primus zkTLS SDK on the backend:
...
- Linea Mainnetï¼š[https://lineascan.build/](https://lineascan.build/)
- BNB Chainï¼š[https://bascan.io/](https://bascan.io/)
- opBNBï¼š[https://scan.sign.global/](https://scan.sign.global/)
- Arbitrumï¼š[https://arbitrum.easscan.org/](https://arbitrum.easscan.org/)
- Scroll Mainnetï¼š[https://scrollscan.com/](https://scrollscan.com/)
...
try {
  ...
  const startAttestaionResult = await sdkInstance.startAttestation({
    ...
    attestationParameters: ["100"], // Input the corresponding fields ï¼ˆif needed) according to the selected attestationTypeID
  });
...
Hereâ€™s a basic example of how to use Primusâ€™ zkTLS SDK from the frontend. Note that this implementation is intended for testing purposes only.
...
- A key issue with AI in decentralized systems is the absence of transparency and accountability. Without verifiable mechanisms, users cannot trust AIâ€™s actions, decisions, or data management, raising concerns about potential manipulation, security vulnerabilities, and privacy breaches.
...
### **Chain-Agnostic**
- Primusâ€™ data verification and computation capabilities are chain-agnostic, enabling dApps across various blockchains to seamlessly interact with cryptographic computations without technical barriers. By bridging the gap between off-chain and on-chain data, Primus empowers blockchain ecosystems to support more diverse and data-driven applications.

### **Openness**
- Primusâ€™ core technologies are open and freely accessible to all developers. Additionally, the network incentivizes contributions from a wide range of communitiesâ€”including cryptography, distributed systems, DeFi, AI, and othersâ€”to collaboratively build a trustless and privacy-centric ecosystem.
...
**How do we share personal informationï¼Ÿ**

We do not share the personal information you provide to us with other organizations without your consent. We only share personal information with third parties under the following circumstancesï¼š
