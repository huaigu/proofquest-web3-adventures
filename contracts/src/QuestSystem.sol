...
contract QuestSystem is Initializable, OwnableUpgradeable, UUPSUpgradeable {
     ...
     */
    function getAllQuestIds(uint256 _offset, uint256 _limit) external view returns (uint256[] memory questIds, uint256 totalCount) {
     ...
     */
    function getQuestsByStatus(QuestStatus _status, uint256 _offset, uint256 _limit) external view returns (uint256[] memory questIds, uint256 totalCount) {
     ...
     */
    function getQuestsBySponsor(address _sponsor, uint256 _offset, uint256 _limit) external view returns (uint256[] memory questIds, uint256 totalCount) {
        // First pass: count total matching quests
        uint256 matchCount = 0;
        for (uint256 i = 1; i < _nextQuestId; i++) {
     ...
     */
    function getQuestsByParticipant(address _user, uint256 _offset, uint256 _limit) external view returns (uint256[] memory questIds, uint256 totalCount) {
        // First pass: count total matching quests
        uint256 matchCount = 0;
        for (uint256 i = 1; i < _nextQuestId; i++) {
     ...
     */
    function getQuestStatistics() external view returns (
        uint256 totalQuests,
        ...
        for (uint256 i = 1; i < _nextQuestId; i++) {
            ...
            if (currentStatus == QuestStatus.Active) {
                activeQuests++;
            } else if (currentStatus == QuestStatus.Ended || currentStatus == QuestStatus.Closed) {
                completedQuests++;
            }
     ...
     */
    function getUserStatistics(address _user) external view returns (
        ...
        for (uint256 i = 1; i < _nextQuestId; i++) {
            if (hasQualified[i][_user]) {
                ...
                if (q.isVesting) {
                    ...
                    pendingVestingRewards += vestedAmount > claimedAmount ? vestedAmount - claimedAmount : 0;
                } else {
     ...
     */
    function canUserClaimReward(uint256 _questId, address _user) external view returns (bool canClaim, string memory reason) {
        Quest storage q = quests[_questId];
     ...
     */
    function _getUpdatedQuestStatus(uint256 _questId) internal view returns (QuestStatus) {
        Quest storage q = quests[_questId];
