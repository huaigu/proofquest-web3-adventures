...
// Chain ID from environment variables (defaults to Monad Testnet)
...
export async function claimRewardWithAttestation(questId: bigint, attestation: unknown) {
  ...
  try {
    ...
    // Try to use wagmi's simulateContract to catch errors before wallet submission
    try {
      ...
      // Check if it's a simulateContract function availability issue
      if (simulationError?.message?.includes('simulateContract is not a function')) {
        console.warn('simulateContract not available, falling back to direct writeContract call');
        
        // Fallback to direct contract call without simulation
        const hash = await writeContract(config, {
          address: QUEST_SYSTEM_ADDRESS,
          abi: QUEST_SYSTEM_ABI,
          functionName: 'claimReward',
